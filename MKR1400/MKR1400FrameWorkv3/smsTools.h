...
void handleCommandSMS(String Command){
  ...
  if ((Command.indexOf("ganancia")>=0 && commandExecuted == false) || (Command.indexOf("signal")>=0 && commandExecuted == false)){
    Serial.println("COMMAND.. Señal de 4G");
    commandExecuted = true;
    String rta = "La ganancia actual de la antena de 4G es:";
    rta = rta + '\n' + iconSenal*100/31 + "%" + '\n' + "d4i:" + myIMEI;
    char nCelu[20];
    lastSMSSenderN.toCharArray(nCelu,20);
    sendSMSDirect(nCelu, rta);
