...
#include "smsTools.h"                         // Envío y Recepción de SMS vía 4G
#include "sh1106I2C.h"                        // Pantalla OLED 128 x 64
#include "gsmTools.h"                         // GSM
#include "mySQL.h"                            // Base de Datos MySql Server
#include "adcTools.h"                         // Lecturas de ADC
#include "flashS.h"                           // Memoria Flash
#include "gpsPlus.h"                          // GPS Físico
...
void setup() {
  ...
  Serial.begin(115200);
  Serial1.begin(9600);                        // GPS - Prendo el HardwareSerial a 9600 baudios 
...
void loop() {
  ...
  if (millis() - ultimoCicloBody >= intervalCicloBody) {
    ...
    //  Handler del GPS GY-NE06NV2
    //
    gpsRead();                                        // Lectura del GPS GY-NE06NV2
    gpsDisplayInfo();
    ...
    // Handler de lectura del GPS GSM
    //
    //myGSMUbicacionGPS();
    //displayUbicacion(myLatitud, myLongitud);
