...
void myGSMUbicacionGPS() {
  if (millis() - ultimoLecturaGPS >= intervalLecturaGPS) {
    ...
    if (location.available()) {
      ...
      myLatitud  = String(location.latitude(), 7);
      myLongitud = String(location.longitude(), 7);
      myLocation = String(location.latitude(), 7) + ", " + String(location.longitude(), 7);
      Serial.print("GSM GPS Location: ");
...
void actualizaClock() {
  ...
  rtcDia  = print2digits(rtc.getDay()) + "/" + print2digits(rtc.getMonth()) + "/" + print2digits(rtc.getYear());
  rtcHora = print2digits(rtc.getHours()) + ":" + print2digits(rtc.getMinutes());
  Serial.print(rtcDia);
  Serial.print(" | ");
...
// Setea el Relojitooooooooo
//
void setClock() {
  Serial.print("getTime: ");
  rtc.setEpoch(gsmAccess.getLocalTime());  // Setea la fecha con el epoch recuperado
  //rtc.setEpoch(gsmAccess.getTime());  // Setea la fecha con el epoch recuperado
...
void modemConnect() {
  ...
  while (!gsmConnected) {
    ...
    displayMessage("Autenticando", "Servicio 4g");
