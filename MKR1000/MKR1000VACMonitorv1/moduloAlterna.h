/*
   Sensor medidor de tension de alterna AC
                  Version 2 
                  04/03/2019
              Proyecto Inversores 
...
(1) -- O se corto la luz o no anda el detector de cruce por cero
(2) -- Si llego aca anda el detector de cruce por cero , pero posiblemente sea de forma erratica o no tenga el duty suficiente
(3) -- NO esta encontrando un cero luego del uno del detector de ceros.. posiblemente el duty es muy elevado
(4) -- El duty del detector de cruce por cero supera los 15mS
(5) -- Verificar valores de las resistencias.. el detector de cruce por cero no funciona correctamente y permite un numero muy pequeño de lecturas por ciclo
...
#define DetectorDeCeros 5                   //Pin digital de entrada del detector de ceros
#define SenalAC         A1                  //Pin de entrada analogico lectura de tension
#define RelacionDeTransformacion 21.536     //Relacion de vueltas del transformador
//#define GananciaDivisor   6.3929          //Ganancia del divisor resistivo (2K2 y 1K)
#define GananciaDivisor   6.20              //Tension de alterna con la que se calibra el sensor (subirlo sube el V)


#define TiempoEntreMediciones 10            //Tiempo de espera entre mediciones (segundos)
#define Tension 3.3                         //Tension con la que alimenta el micro


unsigned long lastTime = 0;                 //Guardo la ultima vez que realize una medicion

float   V = 220;                            //Tension   Alterna
...
//Mido cada cierto tiempo la tension de alterna 
//
float Medicion(){
  ...
  if((millis() - lastTime) >= TiempoEntreMediciones * 1000){
    ...
    while(digitalRead(DetectorDeCeros ) == 0){
      ...
      float tension = (valor * Tension / adcref) * RelacionDeTransformacion * GananciaDivisor;//Traduzco dicha lectura a tension   -- Mapeo los valores analogicos a una tension 0V-5V, Mutiplico por la relacion de transformacion y luego invierto la perdida en el divisor resistivo
