...
#define DEBUG   // Comentar esta línea para versiones en producción 

#define BOTtoken "683684685:AAHB5PXCYebXJlixcET9ux0Kw6tbf68z440"  // BOT Token de Telegram
...
//WiFiSSLClient client;                         //Conexión Cliente SSL para el BOT 
UniversalTelegramBot bot(BOTtoken, client);   //Instancia el BOT
...
long myTickerTime = 10000;   // Retardo entre activaciones del Ticker
long lastTicker = 0;         // Auxiliar contador Ticker

int Bot_mtbs = 1000;         // Retardo entre lecturas del BOT
long Bot_lasttime;           // Auxiliar contador del BOT
...
void setup() {
  ...
  if (mySSID == "" or myPWD == ""){
    Serial.println("Fuiste... la clave o el SIDD estan vacíos te mando al AP ");
    setupAP();
  }
  else{
    needWiFi = true;
...
void loop() {
  ...
  indicadorConexion(); // Rutina Ticker de encendido y apagado LED_BUILTIN

  handlerBot();        // Manejador de mensajes del BOT
    ...
    while (client.connected()) {
      if (client.available()) {
        ...
        if (c == ',' && currentLine.substring(0, 3) == "GET") {
          ...
          //if(mySSID != "" and myPWD != "" and memflash.valid == false){
          ...
          if(mySSID != "" and myPWD != ""){
            ...
            delay(1000);
            //Guarda en la memoria flash ("my_flash_store")
            ...
            Serial.println("Recibí del portal la clave o pwd vacías !!");
            needCredentials = true;
            needWiFi = false;            
          }
          
          client.stop();
          //WiFi.end();        // No sé si debería finalizar la conexión
...
void getWiFi() {
  if (mySSID == "" or myPWD == "") {
    Serial.println("Credenciales vacías");
    needCredentials = true;
    needWiFi = false;
  }
  else{
    ...
    if(WiFi.status() != WL_CONNECTED) {
      Serial.println("Intentando conectar al SSID: ");
...
void printAPStatus() {
  ...
  Serial.print(WiFi.RSSI());
  Serial.println(" dBm");
  Serial.print("Para conectarse el portal vaya a http://");
  Serial.println(ip);
#endif
}
