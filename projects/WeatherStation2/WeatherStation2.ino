...
// Tiene la posibilidad de mostrar el estado de un sensor en ThingSpeak, pero lo comento en esta versión
//
// 
...
// Dejo comentariado el Frame de ThingSpeak porque lo tengo apagado 
...
void setup() {
  ...
  display.clear();
  display.drawString(64, 10, "  WiFi   set up  ");
  ...
  delay(1000);
   ...
   while (WiFi.status() != WL_CONNECTED){
     ...
     if (counter < 100) {
       ...
       display.drawString(64, 10, " Setee el SSID");
       display.display();
     }   
     counter++;
...
void updateData(OLEDDisplay *display) {
  ...
  drawProgress(display, 40, "Actualizando Clima...");
  wunderground.updateConditions(WUNDERGRROUND_API_KEY, WUNDERGRROUND_LANGUAGE, WUNDERGROUND_COUNTRY, WUNDERGROUND_CITY);
  drawProgress(display, 70, "Pronóstico 3 días... ");
...
// Setea el flag para volver a leer la API del pronóstico
//
void setReadyForWeatherUpdate() {
