...
// Rutina groncha de Nicolas para mandar ordenes por el Serial al otro Micro
//
void SwEnviar(int codigo){

  SwSerial.print(codigo);
  SwSerial.println("\n");
   
}


// Rutina groncha de Nicolas para leer por el Serial al otro Micro
//
float SwLeerFLOAT(){
  
  while(SwSerial.available() > 0){
    float val = SwSerial.parseFloat();
    if(SwSerial.read()== '\n'){
       return val;
       break; 
    }
