...
//  SwSerial.begin(115200);
  ...
  Wire.begin();   // Inicializo iC2 para conectar los 2 micros
...
void loop() {
  ...
  // Para pedirle al esclavo que nos transmita el al master
  //
  Wire.requestFrom( 1, 1);
  if(Wire.available() == 1) {   // Byte recibido en serie después del request (ESTAN INVERTIDOS PORQUE ES UNA COLA)
    VIN = Wire.read();          
    Serial.print(VIN);          
    Serial.print("\t");
  }
  ...
  Wire.requestFrom( 1, 1);       // request 4 bytes from slave device #1
  if(Wire.available() == 1) {    // Byte recibido en serie después del request (ESTAN INVERTIDOS PORQUE ES UNA COLA)
    VOUT = Wire.read();          
    Serial.print(VOUT);          
    Serial.print("\t");
...
/*
  if(Wire.available() == 2) {   // Byte recibido en serie después del request (ESTAN INVERTIDOS PORQUE ES UNA COLA)
    IIN = Wire.read();          
    Serial.print(IIN);          
    Serial.print(",");
  }
