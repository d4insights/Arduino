/*
 * Librería de herramientas de conexión a WIFI, recupero de conexiones caídas
 * Portal de seteo inicial de SSID y PWD para guardarlos en la FlashStorage del MKR1010
 * 
 * 
 * 
 * 
...
#define SECRET_apSSID "mkr1010AP"
...
void rutinaSaveCredentials(FSecurity credenciales){
  ...
  if(buttonorder == "Save"){
     wifiSSID.toCharArray(credenciales.mySsid, 50);
     wifiPASS.toCharArray(credenciales.myPwd, 50);
...
void validacionModoDebug(bool modo){
  if(modo){                              // Si estoy en debug salto el seteo de claves, hardcodeandola
    SECRET_routerSSID.toCharArray(credenciales.mySsid, 50);
    SECRET_routerPWD.toCharArray(credenciales.myPwd, 50);
    credenciales.valid  = true;
  } else {
...
bool testInternetConnection(const char* serverTest){
  ...
  if (clientTest.connect(serverTest, 80)) {
    ...
    rta++;                              // Este serviría para un testeo rápido sin llegar a esperar respuesta de la página
    //delay(300);
    clientTest.println("GET  HTTP/1.1");
    clientTest.print("Host: ");
    clientTest.println(serverTest);
...
//  if(rta == 0){
//      delay(800);  // Delay necesario para que la pagina web responda       
//      
